...
/**
 * SynWeb 1.3 : Wall Module (Mobile Version)
 *
 * PHP version 7.3.3
 *
 * @category Sharing
...
if (isset($_GET['nid'])) {
	...
	if (isLoggedIn($logId)){
		...
		while($row = mysqli_fetch_array( $checkComs )) {
			...
			$againChar = mysqli_query($connTemp, "SELECT charid,charname,currency,experience,charimage,id FROM users WHERE charid = '$comsId'") or die(mysqli_error($conn));
			...
			echo "<center><table class='table-results' style='width:98%; margin:1px; display: block;'><th style='font-size:1.2em; width:15%;color:#3A5ECA; max-width: 15%; overflow-wrap: break-word; word-break: break-word; vertical-align:top;'>
			
			<i style='float:left; padding-right:8px;'><img src='./images/user/".$myCharImage1."' alt='".$myCharImage1."' width='80px' height='80px' style='border: 1px solid #3A5ECA;'></img></i>
			...
			<i style='float:left;'><i class='fas fa-link'></i><font size='3' color='#000'>".number_format($thisFollowCount)."</font></i><i style='float:right;'><font size='3' color='#000'> ".$myCharEXP1."</font> <i class='fas fa-flask'></i></i>
			
			</th></table></center>";
			echo "<center><table class='table-results' style='width:98%; margin:1px; display: block;'><th style='width:15%; color:#000; font-size:1.5em; max-width: 15%; overflow-wrap: break-word; word-break: break-word;'>";
			if (!empty($comsCodeUrl)){
				echo "<i style='float:right; font-size:0.5em; color:#3A5ECA;'><a href='./?nid=".$logId."&url=".$comsCodeUrl."' title='Load URL comments?' target='_blank'>Archive Link</a> | ".$comsTimeNew."</i><br />";
				}
			else {
				echo "<i style='float:right; font-size:0.5em; color:#3A5ECA;'>".$comsTimeNew."</i><br />";		
				}
			if (!empty($preVideo)){
				$preVideoMake = substr($preVideo, strrpos($preVideo, '/') + 1);
				echo '<center><div class="stubsize" style="border: 2px solid #3A5ECA; border-radius: 6px; margin-top:10px; margin-bottom:10px;"><div class="youtube" data-embed="'.$preVideoMake.'"><div class="play-button" style="border: 2px solid #3A5ECA; border-radius: 6px; margin-top:10px; margin-bottom:10px;"></div></div></div></center><br />';
				}
			if (!empty($preImage)){
				echo "<center><img src='".$preImage."' alt='".$preImage."' width='98%;' style='border: 2px solid #3A5ECA; height: 30vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></img></center><br />";
				}
			echo "<p style='margin-left: 5px;'>".$comsPost."</p><br /></th></table></center>";		
			...
			echo "<center><table class='table-results' style='width:98%; margin:1px;'><th style='color:#000; font-size:1.5em; max-width: 60%; overflow-wrap: break-word; word-break: break-word;'>";
			...
			echo "<center><table class='table-results' style='width:98%; margin:1px;'><th style='color:#000; font-size:1.5em; max-width: 60%; overflow-wrap: break-word; word-break: break-word; border: 2px solid #36393f;'>";
			...
			while($row = mysqli_fetch_array( $checkReps )) {
				...
				<i style='float:left; padding-right:8px;'><img src='./images/user/".$myCharImage2."' alt='".$myCharImage2."' width='80px' height='80px' style='border: 1px solid #3A5ECA;'></img></i>
				...
				<i style='float:left;'><i class='fas fa-link'></i><font size='3' color='#000'>".number_format($thisFollowCount1)."</font></i><i style='float:right;'><font size='3' color='#000'> ".$myCharEXP2."</font> <i class='fas fa-flask'></i></i>
				...
				if (!empty($comsCodeUrl)){
					echo "<i style='float:right; font-size:0.5em; color:#3A5ECA;'><a href='./?nid=".$logId."&url=".$comsCodeUrl."' title='Load URL comments?' target='_blank'>Archive Link</a> | ".$comsTimeNew1."</i><br />";
					}
				else {
					echo "<i style='float:right; font-size:0.5em; color:#3A5ECA;'>".$comsTimeNew1."</i><br />";		
					}
				if (!empty($preVideo1)){
				...
				if (!empty($preImage1)){
					echo "<center><img src='".$preImage1."' alt='".$preImage1."' width='98%;' style='border: 2px solid #3A5ECA; height: 30vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></img></center><br />";
					}
				echo "<p style='margin-left: 5px;'>".$comsPost1."</p><br /></th></table></center><br />";
