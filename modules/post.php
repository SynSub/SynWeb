...
if (isset($postId)) {
	...
	while($row = mysqli_fetch_array( $checkComs )) {
		...
		if (!empty($preVideo)){
			echo "<center><iframe src='".$preVideo."?controls=1' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen width='98%;' style='border: 2px solid #3A5ECA; height: 60vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></iframe></center>";
			}
		if (!empty($preImage)){
		...
		echo "<i style='float:right; font-size:0.8em; color:#3A5ECA;'>".$comsTimeNew."</i><p style='margin-left: 10px;'>".$comsPost."</p></th></table></center>";
		...
		echo "<center><table class='table-results' style='width:73%; margin:1px; float:right;'><th style='color:#000; font-size:1.5em; max-width: 60%; overflow-wrap: break-word; word-break: break-word; border: 2px solid #36393f;'>";
		...
		while($row = mysqli_fetch_array( $checkReps )) {
			...
			echo "<table class='table-results' style='width:73%; margin:1px; float:right;'><th style='color:#000; font-size:1.5em; max-width: 60%; overflow-wrap: break-word; word-break: break-word;'>";
			if (!empty($preVideo1)){
				echo "<center><iframe src='".$preVideo1."?controls=1' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen width='98%;' style='border: 2px solid #3A5ECA; height: 60vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></iframe></center>";
				}
			if (!empty($preImage1)){
			...
			echo "<i style='float:right; font-size:0.8em; color:#3A5ECA;'>".$comsTimeNew1."</i><p style='margin-left: 10px;'>".$comsPost1."</p></th></table></center>";
