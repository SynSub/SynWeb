...
/**
 * SynWeb 1.3 : Post Module (Mobile Version)
 *
 * PHP version 7.3.3
 *
 * @category Sharing
...
if (isset($postId)) {
	...
	mysqli_real_escape_string($conn, $postId);
	$checkComs = mysqli_query($conn, "SELECT * FROM coms WHERE comcode = '$postId' ORDER BY comtime DESC") or die(mysqli_error($conn));
	while($row = mysqli_fetch_array( $checkComs )) {
		...
		$comsMeta = substr($comsPost, 0, 280)." ... ";
		...
		if (!empty($preVideo)){
			echo "<center><iframe src='".$preVideo."?controls=1' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen width='98%;' style='border: 2px solid #3A5ECA; height: 30vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></iframe></center>";
			}
		if (!empty($preImage)){
		...
		while($row = mysqli_fetch_array( $checkReps )) {
			...
			echo "<center><table class='table-results' style='width:98%; margin:1px;'><th style='width:15%; color:#000; font-size:1.5em; max-width: 15%; overflow-wrap: break-word; word-break: break-word;'>";
			if (!empty($preVideo1)){
				echo "<center><iframe src='".$preVideo1."?controls=1' frameborder='0' allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' allowfullscreen width='98%;' style='border: 2px solid #3A5ECA; height: 30vh; border-radius: 6px; margin-top:10px; margin-bottom:10px;'></iframe></center>";
				}
			if (!empty($preImage1)){
