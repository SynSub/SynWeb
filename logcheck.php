...
function isLoggedIn($nodeProbe) {
	...
	if (isset($nodeProbe)) {
		...
		$resultsTemp = mysqli_query($connTemp, "SELECT nodeid,nodecode FROM node WHERE nodeid = '$tempId' LIMIT 1") or die(mysqli_error($connTemp));
